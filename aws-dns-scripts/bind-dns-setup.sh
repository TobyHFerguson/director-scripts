...
# AWS DNS Assumptions
#
nameserver_ip="169.254.169.253" # used for all regions
...
base_beginning() {
    echo "-- STOP --"
    echo "This script will turn a fresh host into a BIND server and walk you through changing AWS DNS "
...
#
# This function prompts the person running the script to go to the AWS console to change AWS
# DNS settings then makes sure everything works as expected
#
base_end() {
    #
    # Now it's time to update AWS DNS settings in portal
    #
    cat <<EOF
Go to AWS console and make the following changes:
1. Create a set of DHCP options with the following key/value combinations:
   domain-name=$internal_fqdn_suffix
   domain-name-servers=$(hostname -i)
   (See http://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/VPC_DHCP_Options.html#CreatingaDHCPOptionSet)
2. Assign the DHCP option set to the VPC
   (See http://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/VPC_DHCP_Options.html#ChangingDHCPOptionsofaVPC)
EOF
    printf "Press [Enter] once you have completed these tasks"
    read -r
...
original_base_end() {
    ...
    echo ""
    echo "-- STOP -- STOP -- STOP --"
    echo "Go to -- portal.azure.com -- and change AWS DNS to point to the private IP of this host: ${internal_ip}"
...
# if lsb_release isn't installed, use /etc/redhat-release
    ...
    if grep "CentOS.* 7\.[23]" /etc/redhat-release
    then
        os="CentOS"
        release="7.2"
    fi
